...
function PanelHeader({ title, icon, onCollapse, collapseDirection }: PanelHeaderProps) {
  return (
    <div className="h-[40px] px-4 border-b border-macos-gray-200 dark:border-macos-gray-900 flex items-center justify-between bg-white/50 dark:bg-macos-gray-900/50">
      <div className="flex items-center gap-2">
        {icon && <span className="text-sm">{icon}</span>}
        <span className="text-[13px] font-semibold text-macos-gray-700 dark:text-macos-gray-200">{title}</span>
      </div>
